...
# 정전식 터치 센서 4핀 정의 (touch1 Pin 33, touch2 Pin 32, touch3 Pin 35, touch4 Pin 34)
touch1 = Pin(33, Pin.IN, Pin.PULL_DOWN)
touch2 = Pin(32, Pin.IN, Pin.PULL_DOWN)
touch3 = Pin(35, Pin.IN)
touch4 = Pin(34, Pin.IN)

# 터치 센서 에지 검출용 이전 상태 변수
touch1_last = 0
touch2_last = 0
touch3_last = 0
touch4_last = 0
...
# BLE 인스턴스 초기화 및 'ESP_s' 장치명으로 페어링 시작 (웹 검색 필터인 'ESP_' 매칭)
ble = bluetooth.BLE()
p = ble_library.BLESimplePeripheral(ble, "ESP_eun")
...
def on_rx(v): 
    ...
    if v == '9':
        try:
            with open('img/rira.pbm', 'rb') as f:
            ...
            print("Image Load Error (rira.pbm):", e)
            display2.fill(0)
            display2.text("No image found", 8, 28, 1)
            display2.show()
...
while True:
    ...
    if t1 == 1 and touch1_last == 0:
        print("Button 1 touched")
        R.on()
        G.off()
        B.off()
        
    elif t2 == 1 and touch2_last == 0:
        print("Button 2 touched")
        R.on()
        G.on()
        B.off()
    
    elif t3 == 1 and touch3_last == 0:
        print("Button 3 touched")
        R.on()
        G.off()
        B.on()
        
    elif t4 == 1 and touch4_last == 0:
